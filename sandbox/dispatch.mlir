// RUN: iree-opt --pass-pipeline="builtin.module(func.func(iree-llvmcpu-tile{tiling-level=distribution},iree-decompose-aggregated-ops{filter-ops=iree_linalg_ext.exp_reduction}))" --split-input-file %s
...
#qk_config = #iree_cpu.lowering_config<
  vector_common_parallel = [0, 0, 0, 32]
>

#pv_config = #iree_cpu.lowering_config<
  distribution = [32, 16, 0, 0]
>

func.func @attention(%4: tensor<20x4096x64xf16>, %5: tensor<20x4096x64xf16>,  %6: tensor<20x4096x64xf16>) -> tensor<20x4096x64xf16> {
    ...
    %9 = linalg.generic {
        ...
        } ins(%4, %5 : tensor<20x4096x64xf16>, tensor<20x4096x64xf16>) outs(%8 : tensor<20x4096x4096xf32>)
        attrs = {
            lowering_config = #qk_config,
            attention_qk_matmul
        }
        {
    ...
    %16:3 = iree_linalg_ext.exp_reduction{
        indexing_maps = [#map, #map4, #map5, #map5, #map2],
        iterator_types = [
    ...
    // attributes {lowering_config = #pv_config, attention_pv_matmul}
    attributes {
        help = 1: i64
    }
    ins(%10, %6 : tensor<20x4096x4096xf32>, tensor<20x4096x64xf16>) outs(%13, %14, %15 : tensor<20x4096xf32>, tensor<20x4096xf32>, tensor<20x4096x64xf32>) {
    ^bb0(%arg0: f32, %arg1: f16, %arg2: f32, %arg3: f32, %arg4: f32):
        ...
        iree_linalg_ext.yield %arg2, %19, %22 : f32, f32, f32
    ...
    return %18: tensor<20x4096x64xf16>
}
