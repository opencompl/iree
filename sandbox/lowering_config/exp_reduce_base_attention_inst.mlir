...
flow.executable private @executable_0 {
  ...
  builtin.module {
    func.func @dispatch(
      ...
      %S_16_init = tensor.empty() : tensor<4x32x4096x4096xf16>
      %S_16 = linalg.generic {
      indexing_maps = [
            affine_map<(Z, H, N1, N2) -> (Z, H, N1, N2)>,
            affine_map<(Z, H, N1, N2) -> (Z, H, N1, N2)>
      ],
      iterator_types = ["parallel", "parallel",  "parallel", "parallel"]
      }
      ins(%S: tensor<4x32x4096x4096xf32>)
      outs(%S_16_init  : tensor<4x32x4096x4096xf16>)
      {
      ^bb0(%s: f32, %s_16: f16):
        %trunc = arith.truncf %s : f32 to f16
      ...
      %MAX, %SUM, %PV = iree_linalg_ext.exp_reduction {
        ...
        ins(%S_16, %V : tensor<4x32x4096x4096xf16>, tensor<4x32x4096x64xf16>)
