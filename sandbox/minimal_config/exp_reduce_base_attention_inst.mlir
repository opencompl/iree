...
flow.executable private @executable_0 {
  ...
  builtin.module {
    func.func @dispatch(
      ...
      %dispS = stream.binding.subspan %argS[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<4x32x4096x4096xf16>>
      ...
      %S = iree_tensor_ext.dispatch.tensor.load %dispS, offsets = [0,0,0,0], sizes = [4,32,4096,4096], strides = [1,1,1,1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<4x32x4096x4096xf16>> -> tensor<4x32x4096x4096xf16>
      ...
      %MAX, %SUM, %PV = iree_linalg_ext.exp_reduction {
        ...
        ins(%S, %V : tensor<4x32x4096x4096xf16>, tensor<4x32x4096x64xf16>)
...
func.func @attention(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view) -> !hal.buffer_view {
  ...
  %0 = hal.tensor.import %arg0 "s" : !hal.buffer_view -> tensor<4x32x4096x4096xf16>
  %1 = hal.tensor.import %arg1 "v" : !hal.buffer_view -> tensor<4x32x4096x64xf16>

  %ret0 = flow.dispatch @executable_0::@dispatch[%c](%0, %1) : (tensor<4x32x4096x4096xf16>, tensor<4x32x4096x64xf16>) ->  tensor<4x32x4096x64xf32>
