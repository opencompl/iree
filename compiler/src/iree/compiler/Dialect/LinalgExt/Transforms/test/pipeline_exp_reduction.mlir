// RUN: iree-compile --iree-hal-target-device=local --iree-hal-local-target-device-backends=llvm-cpu --iree-llvmcpu-target-cpu=host --compile-from=flow %s  | iree-check-module --device=local-task --module=-
...
flow.executable private @executable_0 {
  ...
  builtin.module {
    func.func @dispatch(
      ...
      %dispQ = stream.binding.subspan %argQ[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<20x4096x64xf32>>
      %dispK = stream.binding.subspan %argK[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<20x4096x64xf32>>
      %dispV = stream.binding.subspan %argV[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<20x4096x64xf32>>
      %dispR = stream.binding.subspan %ret[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<20x4096x64xf32>>

      %Q = iree_tensor_ext.dispatch.tensor.load %dispQ, offsets = [0,0,0], sizes = [20,4096,4096], strides = [1,1,1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<20x4096x64xf32>> -> tensor<20x4096x64xf32>
      %K = iree_tensor_ext.dispatch.tensor.load %dispK, offsets = [0,0,0], sizes = [20,4096,4096], strides = [1,1,1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<20x4096x64xf32>> -> tensor<20x4096x64xf32>
      %V = iree_tensor_ext.dispatch.tensor.load %dispV, offsets = [0,0,0], sizes = [20,4096,4096], strides = [1,1,1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<20x4096x64xf32>> -> tensor<20x4096x64xf32>
      ...
      %S = linalg.generic  {
        ...
        ins(%Q, %K : tensor<20x4096x64xf32>, tensor<20x4096x64xf32>)
        outs(%S_fill : tensor<20x4096x4096xf32>)
      {
      ...
      iree_tensor_ext.dispatch.tensor.store %result, %dispR, offsets = [0,0,0], sizes = [20,4096,64], strides = [1,1,1] : tensor<20x4096x64xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<20x4096x64xf32>>
      return
    }
  }
}
...
func.func @attention(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view, %arg2: !hal.buffer_view) -> !hal.buffer_view {
  ...
  %3 = hal.tensor.export %ret0 "input1" : tensor<20x4096x64xf32> -> !hal.buffer_view 
